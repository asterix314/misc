...
+ Worker资源组：选择“生产环境-worker集群”。可在页面#menu([运维中心], [工作集群监控])中查看计算资源配置。
...
+ 进入#menu([配置中心],[项目空间])页面，点击右上角新建项目空间按钮。
...
平台级角色通常具有更高的权限，使用的功能模块不局限于项目空间，如配置中心、安全中心等。访问位置为#menu([配置中心],[成员角色])。

而项目级角色通常以项目空间为单位进行数据任务的开发，使用的功能也局限在单个空间内，如数据开发、运维中心等。访问位置为#menu([配置中心],[项目空间])，然后编辑项目设置。如@fig:项目角色授权。
...
进入#menu([配置中心],[数据源])页面，点击#menu([新建数据源])，在@fig:编辑数据源 所示界面上填写数据源信息即可。
...
连接配置分为“生产环境”和“开发环境”，这取决于我们之前配置过的项目空间开发模式。但无论哪种开发模式，#highlight[生产环境数据源都是必须的]。#footnote[注意：如果数据源是MySQL且需要实时同步数据（#menu([数据集成],[新建同步任务],[实时同步])），则需提前打开MySQL数据库的binlog相关设置，且提供的数据库账号密码需具备采集binlog的权限。]
