...
如需新建分层，可进入#menu([数据建模], [数仓规划], [分层设计])页面进行操作。当模型设计完成发布物化至数据开发平台时模型将自动发布至分层所绑定的数据库，无需每次发布时都选择数据库。
...
进入#menu([数据建模], [数仓规划], [主题域/应用域])页面可以新建主题域/应用域#footnote[也可选择Excel导入方式]。这些域名可以形成层次结构。子域负责人默认继承上级域的，也可以为子域单独指定负责人。负责人仅能从模型设计师角色成员中选择。
...
“模型设计师”进入#menu([数据建模],[模型设计],[模型设计])页面后可以新建数据模型（@fig:新建模型）。
...
+ 在模型详情或者编辑页面点击#menu([发布])，选择发布环境。通常*先将模型发布至开发环境测试无误后，再发布至生产环境*。
    - #menu([删除重建])：将删除原有模型后重新创建此次发布的模型。原有数据丢失。
    - #menu([增量发布])：相当于执行`ALTER TABLE`，原有数据将被保留。但有些更新不支持，如增加/删除主键列、修改高级设置等。
...
#figure(
    table(
        ...
        [分区字段], [需选择一个`date`或`datetime`类型的字段，且该字段必须被设置为主键列。],
...
- 唯一模型#footnote[唯一模型（Unique Key）能够保证主键的唯一性，当用户更新一条数据时，新写入的数据会覆盖具有相同主键的旧数据。]和聚合模型#footnote[聚合模型（Aggregate Key）：导入数据的时候会按照聚合类型进行聚合，最终只会存储聚合后的数据。]：默认在主键列上分桶。支持设置多列，但必须从主键列中选择。 

- 明细模型#footnote[在明细数据模型中，数据按照导入文件中的数据进行存储，不会有任何聚合。即使两行数据完全相同，也都会保留。]：默认主键列上分桶。如不设置主键，则取前三列，但第一列不能是`float`、`double`、`string`、`array`、`jsonb`类型，如果有则截断。也支持设置其它列（不限于主键列，但不能是限制数据类型）。 
