...
进入#菜单([运维中心],[实时集成任务])页面，在任务运维标签页的列表中找到相关任务，然后在右侧的操作栏中点击#菜单([数据统计])，即可查看历史同步的数据量和同步速度（@fig:实时集成数据统计）等健康参数。
...
*对实时集成任务，应重点关注异常日志*（#菜单([运维中心],[实时集成任务], [异常日志])）。如果报错是暂时性的（如@fig:实时集成日志），则可以不必理会，因为CDC机制会在后续的同步成功时弥补以前的缺失。
...
当源端业务系统的数据或数据结构发生重大变化、或数据由前期试用阶段转为正式投产等特殊情况时，可能想要清空数据平台中已经实时同步的数据，并重新同步全量数据。进入#菜单([运维中心],[实时集成任务])页面，在任务运维标签页的列表中找到相关任务，然后在右侧的操作栏中点击#菜单([重跑])（@fig:重跑实时集成任务）。该操作可将该任务内同步数据的目的表进行清空，然后重新从来源表同步全量数据到目的表中。
